 ...
 * La struttura previene elementi duplicati e si basa sull'ordine naturale dei promemoria,
 * ovvero quello cronologico in base alla data.
 * @author gruppo07
 */
public class ElencoPromemoria extends TreeSet<Promemoria> {

    /**
     * Inserimento controllato di un promemoria nella struttura.Il metodo riceve le informazioni sulla data (giorno, mese, anno, ora, minuti) e sulla descrizione
     * e inserisce un nuovo promemoria se non è già presente uno con la stessa data.
     * Il metodo è thread safe sulla struttura
     ...
     * @throws PromemoriaPresenteException se esiste già un promemoria con quella data
     * @throws DataNonValidaException se la data passata non è valida
     */
    public synchronized void inserisciPromemoria(String descrizione, int giorno, int mese, int anno, int ora, int minuti) throws PromemoriaPresenteException, DataNonValidaException {
        try{
    ...
    /**
     * Rimozione controllata di un promemoria dalla struttura dati,
     * controllando l'esistenza del promemoria da rimuovere.
     * Il metodo è thread safe
     * @param p promemoria da rimuovere
     * @throws PromemoriaNonEsistenteException se il promemoria non è presente nella struttura dati
     */
